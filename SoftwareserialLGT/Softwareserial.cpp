...
// (courtesy of Arduino forum user *etracer*)
	...
	if (inverse_logic ? rx_pin_read() : !rx_pin_read())
	{
	  ...
	  // cause problems at higher baudrates.
		...
		uint8_t next = (_receive_buffer_tail + 1) & _SS_MAX_RX_BUFF - 1 /*% _SS_MAX_RX_BUFF*/;
		if (next != _receive_buffer_head)
		{
		  // save new data in buffer: tail points to where byte goes
			_receive_buffer[_receive_buffer_tail] = inverse_logic ? ~d : d; // save new byte
...
void SoftwareSerial::setTX(uint8_t tx)
{
	...
	digitalWrite(tx, inverse_logic ? LOW : HIGH);
	if (openDrainMode) {
		pinMode(tx, inverse_logic ? OUTPUT : INPUT);
		_transmitPortRegister = portModeRegister(port);
	} else {
		pinMode(tx, OUTPUT);
...
{
	pinMode(rx, INPUT);
	if (rxPullUp && !openDrainMode && !inverse_logic)
		digitalWrite(rx, HIGH);  // pullup for normal logic!
	else digitalWrite(rx, LOW);
	_receivePin = rx;
...
int SoftwareSerial::read()
{
	...
	_receive_buffer_head = (_receive_buffer_head + 1) & _SS_MAX_RX_BUFF - 1/*% _SS_MAX_RX_BUFF*/;
	return d;
}
...
int SoftwareSerial::available()
{
	...
	return (_receive_buffer_tail + _SS_MAX_RX_BUFF - _receive_buffer_head) & _SS_MAX_RX_BUFF - 1 /*% _SS_MAX_RX_BUFF*/;
}
...
size_t SoftwareSerial::write(uint8_t b)
{
	...
	// critical timing sections below, which makes it a lot easier to verify the cycle timings
	volatile uint8_t* reg = _transmitPortRegister;
	const uint8_t inv = openDrainMode ^ inverse_logic,
	reg_mask = _transmitBitMask,
	inv_mask = ~_transmitBitMask,
	oldSREG = SREG;
	const uint16_t delay = _tx_delay;
